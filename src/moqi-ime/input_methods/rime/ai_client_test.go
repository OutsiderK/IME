...
func TestGenerateInlineCompletionsUsesFewShotAndLocalRepeatControls(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"choices":[{"message":{"role":"assistant","content":"柑橘\n莓果\n蜂蜜"}}]}`))
	}))
	defer server.Close()
	client := &aiClient{baseURL: server.URL, apiKey: "local", model: "qwen", httpClient: server.Client()}

	candidates, err := client.GenerateInlineCompletions(aiCompletionRequest{
		Context: "具体有番茄、", FollowingContext: "", Long: false,
	}, aiCompletionConfig{ContextTokens: 128, CandidateCount: 3, MaxOutputTokens: 48, Temperature: 0.2})
...
func TestGenerateInlineCompletionsUsesInfillPromptWhenFollowingTextExists(t *testing.T) {
	var captured chatCompletionsRequest
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if err := json.NewDecoder(r.Body).Decode(&captured); err != nil {
			t.Fatalf("decode request: %v", err)
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"choices":[{"message":{"role":"assistant","content":"来自埃塞俄比亚的\n一杯\n水洗处理的"}}]}`))
	}))
	defer server.Close()
	client := &aiClient{baseURL: server.URL, apiKey: "local", model: "qwen", httpClient: server.Client()}

	candidates, err := client.GenerateInlineCompletions(aiCompletionRequest{
		Context: "今天喝了", FollowingContext: "的耶加雪菲，香气浓郁。", Long: true,
	}, aiCompletionConfig{ContextTokens: 192, CandidateCount: 3, MaxOutputTokens: 48, Temperature: 0.25})
	if err != nil {
		t.Fatalf("unexpected infill error: %v", err)
	}
	want := []string{"来自埃塞俄比亚", "水洗处理"}
	if len(candidates) != len(want) || candidates[0] != want[0] || candidates[1] != want[1] {
		t.Fatalf("got %#v want %#v", candidates, want)
	}
	if !strings.Contains(captured.Messages[0].Content, "句中填空") || !strings.Contains(captured.Messages[len(captured.Messages)-1].Content, "禁止数量词") {
		t.Fatalf("expected dedicated infill prompt, got %#v", captured.Messages)
	}
	if captured.MaxTokens < 64 {
		t.Fatalf("expected enough output budget for infill candidates, got %d", captured.MaxTokens)
	}
}

func TestLoopbackAIEndpointDetection(t *testing.T) {
	for _, endpoint := range []string{"http://127.0.0.1:8080/v1", "http://localhost:8080/v1", "http://[::1]:8080/v1"} {
		if !isLoopbackAIEndpoint(endpoint) {
			t.Fatalf("expected loopback endpoint %q", endpoint)
		}
	}
	if isLoopbackAIEndpoint("https://api.example.com/v1") {
		t.Fatal("expected remote endpoint not to receive llama.cpp-only fields")
	}
}
